...
`null` y `campos_ilegibles` vacio. No inventes un total plausible para una fotografia
de un perro.
...
El campo `confianza_0_1` no lo produce el modelo por magia. Lo produce porque el prompt
definio una escala con criterios observables. Sin esa escala, el numero es decorativo y
el `Guardrail` que lo usa como umbral es teatro.

Pregunta para la puesta en comun: si su prompt no hubiese incluido la tabla del Paso 3,
que significaria un 0,85 devuelto por el modelo?
